// Warmup loop; time N evaluations; report mean and p50. (Stage-1 is about correctness + package,
// not perf heroics yet.)
#include "bs/bs.hpp"
#include <chrono>
#include <cstdlib>
#include <iostream>
...
static double parse_or(const char *s, double d) {
  try {
    return s ? std::stod(s) : d;
  } catch (...) {
    return d;
  }
}
static long parse_or_long(const char *s, long d) {
  try {
    return s ? std::stol(s) : d;
  } catch (...) {
    return d;
  }
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    std::string a = argv[i];
    auto next = [&](double &ref) {
      if (i + 1 < argc)
        ref = parse_or(argv[++i], ref);
    };
    if (a == "--type" && i + 1 < argc)
      type = argv[++i];
    else if (a == "--iters" && i + 1 < argc)
      iters = parse_or_long(argv[++i], iters);
    else if (a == "--S")
      next(p.S);
    else if (a == "--K")
      next(p.K);
