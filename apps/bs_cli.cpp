...
namespace {
    ...
    void print_usage(const char* prog) {
        std::cerr
          ...
          << "Example:\n  " << prog << " --type call --S 100 --K 100 --r 0.05 --sigma 0.2 --T 1\n";
    }

    bool parse_double(const std::vector<std::string>& args, size_t& i, double& out) {
        if (i + 1 >= args.size()) return false;
        try { out = std::stod(args[++i]); } catch (...) { return false; }
        return true;
    }
...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < args.size(); ++i) {
        ...
        if (a == "--type") {
            if (i + 1 >= args.size()) { print_usage(argv[0]); return 2; }
            type = args[++i];
        } else if (a == "--S") {
            if (!parse_double(args, i, p.S)) { print_usage(argv[0]); return 2; }
        } else if (a == "--K") {
            if (!parse_double(args, i, p.K)) { print_usage(argv[0]); return 2; }
        } else if (a == "--r") {
            if (!parse_double(args, i, p.r)) { print_usage(argv[0]); return 2; }
        } else if (a == "--sigma") {
            if (!parse_double(args, i, p.sigma)) { print_usage(argv[0]); return 2; }
        } else if (a == "--T") {
            if (!parse_double(args, i, p.T)) { print_usage(argv[0]); return 2; }
        } else if (a == "--help" || a == "-h") {
    ...
    if (type.empty()) { std::cerr << "--type is required\n"; print_usage(argv[0]); return 2; }
    std::string why;
